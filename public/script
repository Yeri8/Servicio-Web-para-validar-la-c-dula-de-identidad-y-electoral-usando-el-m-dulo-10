...
function loadSection(section) {
  ...
  if (section === 'home') {
    main.innerHTML = `<div class="card"><h2>Bienvenido</h2>
    ...
    document.getElementById('serviceBtn').addEventListener('click', async () => {
      const v = document.getElementById('cedInput').value.trim();
      if (!v) {
        document.getElementById('result').textContent = 'Introduce una cédula para validar';
        document.getElementById('result').className = 'result bad';
        return;
      }
      try {
        // Llamada a servicio real: GET /api/validate?cedula=...
        const q = '/api/validate?cedula=' + encodeURIComponent(v);
        const res = await fetch(q, { method: 'GET' });
        if (!res.ok) {
          const txt = await res.text();
          document.getElementById('result').textContent = 'Error del servicio: ' + res.status + ' ' + txt;
          document.getElementById('result').className = 'result bad';
          return;
        }
...
function showResult(r) {
  ...
  if (!r) { el.textContent = 'Error'; el.className = 'result bad'; return; }

  // Si viene del validador cliente, estructura: {valid, reason, ...}
  // Si viene del API, estructura: { valid, message, ... } o { valid:false, ...}
